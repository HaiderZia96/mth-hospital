            ...
            <form method="{{$method}}" action="{{$action}}" enctype="{{$enctype}}">
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('title') is-invalid @enderror"
                                   name="title"
                                   id="title" placeholder="Title" value="{{(!empty($data->title) && isset($data->title)) ? $data->title : old('title')}}" disabled>
                            @error('title')
                    ...
                    <div class="mb-3">
                        ...
                        <textarea disabled id="description" class="form-control mb-0 @error('description') is-invalid @enderror" rows="6" cols="60"  name="description">{!!  strip_tags($data->description) !!}</textarea>
                        @error('description')
                        <strong class="text-danger">{{ $message }}</strong>
                        @enderror
                    </div>
