...
@endsection
@push('head-scripts')
    <link rel="stylesheet" href="{{ asset('department/datatable/datatables.min.css') }}" rel="stylesheet" />
@endpush
@section('content')
    <div class="card mt-3">
        <div class="card-body">
...
{{--                    @can('department_user-management_module-create')--}}
                        <a href="{{(!empty($url) && isset($url)) ? $url : ''}}" class="btn btn-sm btn-primary">{{(!empty($url_text) && isset($url_text)) ? $url_text : ''}}</a>
{{--                    @endcan--}}
{{--                    @can('department_user-management_module-activity-log-trash')--}}
...
@endsection
@push('footer-scripts')
    <script type="text/javascript" src="{{ asset('department/datatable/datatables.min.js')}}"></script>
    <script type="text/javascript">
        $(document).ready(function(){
            ...
            $('#indextable').DataTable({
                ...
                ajax: {
                    "type":"GET",
                    "url":"{{route('department.get.dept-banner')}}",
                    // "data":function (d){
                ...
                columnDefs: [
                    ...
                    {
                        render: function (data,type,row,meta) {
                            var Images ='{{ asset("storage/app/private/Department/DeptBanner")}}/' +data.image;
                            if(data.image){
                                return '<div class="text-center"><img src="' + Images + '" height="50" width="50" alt="No Image Uploaded"/></div>';
                            }else{
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row, meta) {
                            ...
                            let active = "{{ route('department.dept-banner-status',  ':id') }}";
                            active = active.replace(':id', row.id);
                    ...
                    },
                    {
                        ...
                        orderable: false,
                        render: function (data, type, row, meta) {
                            let show = "{{ route('department.dept-banner.show', ':id') }}";
                            show = show.replace(':id', row.id);

                            let edit = "{{ route('department.dept-banner.edit', ':id') }}";
                            edit = edit.replace(':id', row.id);

                            let del = "{{ route('department.dept-banner.destroy',  ':id') }}";
                            del = del.replace(':id', row.id);

                            let ACTIVITY = "{{ route('department.get.dept-banner-activity', ':id') }}";
                            ...
                            if (data.status == '0') {
                                return '<div class="d-flex">' +
{{--                                    @can('manager_frontend_hero-banner-edit')--}}
                                        '<a class="me-1" href="' + active + '"><span class="badge bg-warning text-dark">In Active</span></a> |' +
{{--                                    @endcan--}}
{{--                                        @can('manager_frontend_hero-banner-show')--}}
                                        '<a class="me-1 ms-2" href="' + show + '"><span class="badge bg-success text-dark">Show</span>' +
{{--                                    @endcan--}}
{{--                                        @can('manager_frontend_hero-banner-edit')--}}
                                        '<a class="me-1" href="' + edit + '"><span class="badge bg-info text-dark">Edit</span></a>' +
{{--                                    @endcan--}}
{{--                                        @can('manager_frontend_hero-banner-delete')--}}
                                        '<a class="me-2" href="javascript:void(0)"><span type="button" class="badge bg-danger" data-url="' + del + '" data-coreui-toggle="modal" data-coreui-target="#del-model">Delete</span></a>' +
{{--                                    @endcan--}}
{{--                                        @can('manager_frontend_hero-banner-activity-log')--}}
...
{{--                                    @endcan--}}
                                ...
                                return '<div class="d-flex">' +
{{--                                    @can('manager_frontend_hero-banner-edit')--}}
                                        '<a class="me-1" href="' + active + '"><span class="badge bg-success text-white">Active</span></a> |' +
{{--                                    @endcan--}}
{{--                                        @can('manager_frontend_hero-banner-show')--}}
                                        '<a class="me-1 ms-2" href="' + show + '"><span class="badge bg-success text-dark">Show</span>' +
{{--                                    @endcan--}}
