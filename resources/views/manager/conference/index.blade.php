    ...
    <script type="text/javascript">
        $(document).ready(function(){
            ...
            $('#indextable').DataTable({
                ...
                ajax: {
                    "type":"GET",
                    "url":"{{route('manager.get.conference')}}",
                    // "data":function (d){
                ...
                columnDefs: [
...

                    {
                        ...
                        orderable: false,
                        render: function (data, type, row, meta) {
                            let show = "{{ route('manager.conference.show', ':id') }}";
                            show = show.replace(':id', row.id);

                            let edit = "{{ route('manager.conference.edit', ':id') }}";
                            edit = edit.replace(':id', row.id);

                            let del = "{{ route('manager.conference.destroy',  ':id') }}";
                            del = del.replace(':id', row.id);

                            let ACTIVITY = "{{ route('manager.get.conference-activity', ':id') }}";
                            ACTIVITY = ACTIVITY.replace(':id', row.id);

                            var up = '{{ route("manager.swap-up.conference", [":id"]) }}';
                            up = up.replace(':id', data.id);

                            var down = '{{ route("manager.swap-down.conference", [":id"]) }}';
                            down = down.replace(':id', data.id);



                            return '<div class="d-flex">' +
                                @can('manager_event_conference-swap')
                                ...
                                @endcan
                                    @can('manager_event_conference-activity-log')
