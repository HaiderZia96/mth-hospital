    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('#indextable').DataTable({
                ...
                ajax: {
                    "type": "GET",
                    "url": "{{route('manager.get.research')}}",
                },
                columns: [
                    {data: 'id'},
                    {data: 'title'},
                ...
                columnDefs: [
                    ...
                    },
                    {
                        ...
                        orderable: false,
                        render: function (data, type, row, meta) {
                            let URL = "{{ route('manager.research.show', ':id') }}";
                            URL = URL.replace(':id', row.id);

                            let ACTIVITY = "{{ route('manager.get.research-activity', ':id') }}";
                            ACTIVITY = ACTIVITY.replace(':id', row.id);

                            var up = '{{ route("manager.swap-up.research", [":id"]) }}';
                            up = up.replace(':id', data.id);

                            var down = '{{ route("manager.swap-down.research", [":id"]) }}';
                            down = down.replace(':id', data.id);

                            return '<div class="d-flex">' +
                                @can('manager_research_research-swap')
                                ...
                                @endcan
                                    @can('manager_research_research-show')
                                    '<a class="me-1 ms-2" href="' + URL + '"><span class="badge bg-success text-dark">Show</span>' +
                                @endcan
                                    @can('manager_research_research-edit')
                                    '<a class="me-1" href="' + URL + '/edit"><span class="badge bg-info text-dark">Edit</span></a>' +
                                @endcan
                                    @can('manager_research_research-delete')
                                    '<a class="me-2" href="javascript:void(0)"><span type="button" class="badge bg-danger" data-url="' + URL + '" data-coreui-toggle="modal" data-coreui-target="#del-model">Delete</span></a>' +
                                @endcan
                                    @can('manager_research_research-activity-log')
