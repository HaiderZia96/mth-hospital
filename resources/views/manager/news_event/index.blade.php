    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('#indextable').DataTable({
                ...
                ajax: {
                    "type": "GET",
                    "url": "{{route('manager.get.news-event')}}",
                },
                columns: [
                    {data: 'id', orderable: false},
                    {data: 'name'},
                    {data: 'dpt_id', orderable: false},
                    {data: 'eCateID', orderable: false},
                    {data: 'e_date'},
                    {data: null},
                ],
                columnDefs: [
                    ...
                    },
                    {
                        ...
                        orderable: false,
                        render: function (data, type, row, meta) {
                            let URL = "{{ route('manager.news-event.show', ':id') }}";
                            URL = URL.replace(':id', row.id);

                            let ACTIVITY = "{{ route('manager.get.news-event-activity', ':id') }}";
                            ACTIVITY = ACTIVITY.replace(':id', row.id);

                            var up = '{{ route("manager.swap-up.news-event", [":id"]) }}';
                            up = up.replace(':id', data.id);

                            var down = '{{ route("manager.swap-down.news-event", [":id"]) }}';
                            down = down.replace(':id', data.id);


                            return '<div class="d-flex">' +
                                @can('manager_event_news-swap')
                                ...
                                @endcan
                                    @can('manager_event_news-activity-log')
