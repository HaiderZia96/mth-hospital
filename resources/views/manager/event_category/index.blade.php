    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('#indextable').DataTable({
                ...
                ajax: {
                    "type": "GET",
                    "url": "{{route('manager.get.event-category')}}",
                },
                columns: [
                ...
                columnDefs: [
...

                    {
                        ...
                        render: function (data, type, row, meta) {
                            ...
                            let URL = "{{ route('manager.event-category.show', ':id') }}";
                            URL = URL.replace(':id', row.id);


                            let ACTIVITY = "{{ route('manager.get.event-category-activity', ':id') }}";
                            ACTIVITY = ACTIVITY.replace(':id', row.id);


                            return '<div class="d-flex">' +

                                @can('manager_event_category-show')
                                    '<a class="me-1 ms-2" href="' + URL + '"><span class="badge bg-success text-dark">Show</span>' +
                                @endcan
                                    @can('manager_event_category-edit')
                                    '<a class="me-1" href="' + URL + '/edit"><span class="badge bg-info text-dark">Edit</span></a>' +
                                @endcan
                                    @can('manager_event_category-delete')
                                ...
                                @endcan
                                    @can('manager_event_category-activity-log')
